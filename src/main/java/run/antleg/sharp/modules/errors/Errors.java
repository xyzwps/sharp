...
public enum Errors {
    ...
    USERNAME_CONFLICT("用户名已存在", BAD_REQUEST),
    ...
    TAG_UNSUPPORTED("暂不支持此标签", BAD_REQUEST),

    REQUEST_INVALID("请求错误", BAD_REQUEST),
    REQUEST_UNAUTHORIZED("认证失败", UNAUTHORIZED),
    REQUEST_FORBIDDEN("未授权", FORBIDDEN),

    UNKNOWN_AUTHENTICATION("暂不支持的认证信息", UNAUTHORIZED),
